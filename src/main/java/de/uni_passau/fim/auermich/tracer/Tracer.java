...
public class Tracer extends BroadcastReceiver {
    ...
    private static boolean isPermissionGranted(Context context, final String permission) {
        ...
        if (context == null) {
            ...
            // For API 23+ use ContextCompat
            if (Build.VERSION.SDK_INT >= 23) {
                try {
                    // Use reflection to avoid direct dependency on androidx
                    Class<?> contextCompat = Class.forName("androidx.core.content.ContextCompat");
                    return (int) contextCompat.getMethod("checkSelfPermission", Context.class, String.class)
                            .invoke(null, context, permission) == PERMISSION_GRANTED;
                } catch (Exception e) {
                    // Fallback to old permission check
                    return context.getApplicationInfo().targetSdkVersion < 23 
                           || context.getApplicationInfo().permission == permission;
                }
            }
